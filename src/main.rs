...
fn main() {
    ...
    let _s4 = "(((\\id1.(t1 id1)) (\\id2.(t1 t2))) ((\\id1.(t1 id1)) (\\id2.(t1 t2))))";
    ...
    // Note: _s4 results in a loop or stack overflow.
    // println!("\n\n Testing on:\n{}\nReduce cbv\n", _s4);
    // evaluate(reduce_cbv, parse(_s4));
    ...
    let s7 = "((\\x.x) (\\y.y))";
    println!("\n\n Testing on:\n{}\nReduce cbn\n", s7);
    evaluate(reduce_cbn, parse(s7));

    let s8 = "((\\x.(\\y.y)) ((\\id.(id id)) (\\id.(id id))))";
    println!("\n\n Testing on:\n{}\nReduce cbn\n", s8);
    evaluate(reduce_cbn, parse(s8));

    let s9 = "((\\f.(\\x.(f (f x)))) (\\id.id))";
    println!("\n\n Testing on:\n{}\nReduce cbn\n", s9);
